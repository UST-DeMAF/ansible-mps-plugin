...
public class ActionParser {
    ...
    private Module parseDockerImage(Map<String, Object> taskYaml) {
        ...
        return new DockerImage(
                dockerImageYaml.get("name"),
                dockerImageYaml.get("source")
        );
    }

    private DockerContainer parseDockerContainer(Map<String, Object> taskYaml) {

        Map<String, Object> dockerContainerYaml = (Map<String, Object>) taskYaml.get("docker_container");
        ...
        // Extract networks
        ArrayList<Map<String, String>> networksYaml = (ArrayList<Map<String, String>>) dockerContainerYaml.get("networks");
        HashSet<Variable> networks = networksYaml.stream().map(map -> new Variable(map.keySet().stream().findFirst().get(), map.values().stream().findFirst().get())).collect(Collectors.toCollection(HashSet<Variable>::new));

        // Extract Log options
        Map<String, String> logOptionsYaml = (Map<String, String>) dockerContainerYaml.get("log_options");
