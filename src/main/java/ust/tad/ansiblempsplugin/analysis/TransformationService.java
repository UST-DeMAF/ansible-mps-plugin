...
public class TransformationService {
  ...
  /**
   * Transforms given the internal ansible model to an EDMM model. Uses the MPS project for a
   * model-to-model transformation. In the first step, creates a file containing the tsdm model in
   * the MPS ansible language from the given internal ansible model. Then, the MPS transformation is
   * run, using the Gradle build scripts. After that, the resulting EDMM model is imported and added
   * to the already existing technology-agnostic deployment model. If configured, provider-specific
   * post-processors are run.
   *
  ...
  public TechnologyAgnosticDeploymentModel transformInternalToTADM(
      TechnologyAgnosticDeploymentModel tadm, final AnsibleDeploymentModel ansibleDeploymentModel)
      throws IOException {
  ...
  /**
   * Output the ansible Deployment Model to an XML file on the file system using the Jackson
   ...
   */
  private void createMPSAnsibleDeploymentModel(final AnsibleDeploymentModel ansibleDeploymentModel)
      throws IOException {
